...
- Policy and procedure documentation accessible in `{baseDir}/docs/` (information security policy, incident response plan, BCP/DR plan, vendor management procedures)
...
- Write permissions for audit workspace in `{baseDir}/soc2-audit/`
...
6. **Collect evidence artifacts**: organize evidence into the standard directory structure under `{baseDir}/soc2-audit/` with subdirectories per criteria (CC1-control-environment/, CC6-access-controls/, CC7-system-operations/, etc.).
...
9. **Generate readiness report**: produce `{baseDir}/soc2-audit/readiness-report-YYYYMMDD.md` with overall readiness score, per-criteria assessment with percentage, remediation roadmap with timelines, and evidence collection checklist.
...
See `{baseDir}/references/implementation.md` for the six-phase implementation guide. See `{baseDir}/references/readiness-by-trust-service-category.md` for example per-criteria readiness breakdowns.

## Output

- **Readiness Report**: `{baseDir}/soc2-audit/readiness-report-YYYYMMDD.md` with overall score and per-criteria pass/gap status
...
|-------|-------|----------|
| Cannot locate security policy in `{baseDir}/docs/` | Documentation stored elsewhere or not yet created | Request document locations; flag as critical evidence gap requiring immediate creation |
...
| Production and dev configs mixed in `{baseDir}/` | Environment separation unclear | Request environment labeling; risk of auditing wrong environment |
...
- `{baseDir}/references/readiness-by-trust-service-category.md` -- example per-criteria readiness breakdown
